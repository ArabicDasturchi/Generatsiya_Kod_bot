...
const ADMIN_ID = process.env.ADMIN_ID; // Optional: set in .env to enable admin stats
...
// Session storage (local database in sessions.json)
...
/**
 * Splits long messages into chunks to respect Telegram's 4096 character limit.
 */
...
bot.hears(['🎨 UI/UX Dizayner', '🎨 UI/UX Designer'], (ctx) => {
    ...
    ctx.reply('🎨 Rejim o\'zgartirildi: UI/UX Dizayner (Hamma narsani dizayn va CSS nuqtai nazaridan ko\'raman)');
});
...
// Groq doesn't support video yet, so we'll add a friendly message
bot.on('video', async (ctx) => {
    ctx.reply('⚠️ Hozircha Groq AI video tahlilini qo\'llab-quvvatlamaydi. Iltimos, rasm yoki matn yuboring.');
});

// Document/File as image
